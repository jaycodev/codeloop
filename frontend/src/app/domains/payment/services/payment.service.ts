...
export class PaymentService {
  ...
  getList(): Observable<PaymentListDto[]> {
    return this.http
      .get<ApiResponse<PaymentListDto[]> | ApiError | PaymentListDto[]>(this.baseUrl)
      .pipe(
        map((res) => {
          if (Array.isArray(res)) return res as PaymentListDto[];
          if (
            (res as ApiResponse<PaymentListDto[]>).success === true &&
            (res as ApiResponse<PaymentListDto[]>).data
          )
  ...
  getInfoById(id: number): Observable<PaymentDetailDto> {
    ...
    return this.http.get<ApiResponse<PaymentDetailDto> | ApiError | PaymentDetailDto>(url).pipe(
      map((res) => {
        if (
          (res as ApiResponse<PaymentDetailDto>).success === true &&
          (res as ApiResponse<PaymentDetailDto>).data
        )
